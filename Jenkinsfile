pipeline {
    ...
    environment {
        IMAGE_NAME = "abakhar217/discovery-server:discovery-server-${BUILD_NUMBER}"
        ...
        stage('Checkout') {
            steps {
                ...
                checkout([$class: 'GitSCM',
                          ...
                          userRemoteConfigs: [[url: 'https://github.com/InvestBuddy/discovery-server.git', credentialsId: 'git']]])
            }
        }
        ...
        stage('Check if JAR exists') {
            steps {
                script {
                    ...
                    if (!fileExists('target/discovery-server-1.0-SNAPSHOT.jar')) {
                        error "discovery-server-1.0-SNAPSHOT.jar not found! Build failed."
                    }
                }
            }
        }
			 ...
			 stage('Deploy to Kubernetes') {
			     steps {
			        script {
			          ...
			          if (isUnix()) {
			             sh "sed -i 's#<BUILD_NUMBER>#${BUILD_NUMBER}#g' discovery-server.yml"
			           } 
				   else {
			                bat "powershell -Command \"(Get-Content discovery-server.yml) -replace '<BUILD_NUMBER>', '${BUILD_NUMBER}' | Set-Content discovery-server.yml\""
			            }
			
			            withKubeConfig([credentialsId: 'kubectl']) {
			              if (isUnix()) {
			                sh 'kubectl apply -f discovery-server.yml'
			             } else {
			                bat 'kubectl apply -f discovery-server.yml'
			               }
			             }
			          }
			     }
